# runny TUI-Only Functional Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Implement the approved runny functional scope: TUI-only entrypoint, task/status summary, search, target details/logs, command palette, and keymap.
...
- Modify `internal/tui/model.go`: add command palette mode, target output scroll, status stats, complete help rows, home/end navigation, `x` cancellation, filter clear.
- Modify `internal/tui/model_test.go`: add component tests for status summary, command palette, keymap, target output scroll, TUI-only shortcuts.
- Modify `internal/tui/program_test.go`: extend E2E to cover `/`, `:workers 1`, `?`, `enter`, target output.
- Modify TUI golden testdata: update main functional golden.
...
- [x] Add tests for `g`, `G`, `x`, `ctrl+u` in filter, and target output scroll keys.
- [x] Add fields to `Model` for palette input, palette cursor, target output scroll, and log follow.
- [x] Route global keys before focus-specific handlers.
- [x] Implement palette command parsing and state changes.
- [x] Implement target output scroll helpers.
- [x] Run targeted TUI model tests for palette, target output scroll, and navigation.

## Task 3: Functional TUI Render

- [x] Update golden expected content to contain counters: queued, running, succeeded, failed, cancelled, workers, mode.
- [x] Render status summary.
- [x] Rename left pane to `Tasks`.
- [x] Render focused target details with path, status, command, selection state, and output/logs.
...
- [x] Run main TUI golden test.

## Task 4: E2E And Runtime Proof

- [x] Extend `TestRunnyTUIProgramEndToEnd` to send `/`, `:workers 1`, `?`, `enter`, and verify target output.
...
- [x] Run final `go test ./...`.
- [ ] Commit as `feat(tui): implement tui-only functional scope`.
- [ ] Push `main`.

## Self-Review

- Spec coverage: CLI-only launches TUI, status summary, task list, target details/logs, keymap, filter, palette, run controls, tests, and docs are covered.
